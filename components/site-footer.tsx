  ...
  return (
    <footer className="border-border bg-card mt-auto border-t">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 md:grid-cols-6 lg:px-8">
        ...
        {columns.map((col) => (
          <nav key={col.title} aria-labelledby={`footer-${col.title}`}>
            <h3
              id={`footer-${col.title}`}
              className="text-foreground mb-3 text-xs font-semibold tracking-wider uppercase"
            >
              {col.title}
            </h3>
      ...
      <div className="border-border border-t">
        <div className="text-muted-foreground mx-auto flex max-w-7xl flex-col items-start justify-between gap-2 px-4 py-4 text-xs md:flex-row md:items-center lg:px-8">
